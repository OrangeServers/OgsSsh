...
class PrivateKey(object):
    ...
    # 修改处 变更为传递key文件路径读取key
    def get_specific_pkey_file(self):
        name = 'RSA'
        pkeycls = getattr(paramiko, name + 'Key')
        pkey = None

        try:
            pkey = paramiko.RSAKey.from_private_key_file(self.privatekey)
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def get_args(self):
        ...
        if privatekey:
            ...
            pkey = PrivateKey(privatekey, passphrase, filename).get_specific_pkey_file()
        else:
            pkey = None
