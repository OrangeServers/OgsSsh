...
jQuery(function($){
  ...
  function current_geometry(term) {
    ...
    // 修改处，修改终端宽度
    var cols = parseInt(window.innerWidth / style.width - 30, 10) - 1;

    // 修改处，修改终端高度
    var rows = parseInt(window.innerHeight / style.height - 5, 10);
  ...
  function log_status(text, to_populate, termid, csname) {
    // 修改处，添加错误提示
    $("." + csname).css("color", "red")
    $("#" + termid).html('登录失败：' + text)
    layer.msg('登录失败：' + text)
  ...
  function ajax_complete_callback(resp, termid, csname) {
    button.prop('disabled', false);

    if (resp.status !== 200) {
      log_status(resp.status + ': ' + resp.statusText, true, termid, csname);
      state = DISCONNECTED;
      return;
    }
    ...
    if (!msg.id) {
      log_status(msg.status, true, termid, csname);
      state = DISCONNECTED;
      return;
    }
    ...
    var ws_url = window.location.href.split(/\?|#/, 1)[0].replace('http', 'ws'),
        ...
        // 修改处  新增打开tab页面id传参
        terminal = document.getElementById(termid),
        termOptions = {
          cursorBlink: true,
          theme: {
  ...
  function connect_with_options(data, termid, csname) {
    ...
    $.ajax({
      ...
      // 修改处  新增打开tab页面id传参
        complete: function (res){
          ajax_complete_callback(res, termid, csname)
        },
    });
  ...
  function connect(termid, csname, hostname, port, username, password, privatekey, passphrase, totp) {
    // for console use
    var result, opts;
    ...
    if (hostname === undefined) {
      ...
      // 修改处  新增打开tab页面id传参
      result = connect_with_options(opts,termid,csname);
    }
